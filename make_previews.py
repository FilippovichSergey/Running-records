...
def read_dims() -> dict:
    """The entries already in photo-dims.js (its object literal is plain JSON)."""
    try:
        m = re.search(r"const PHOTO_DIMS = (\{.*\});", DIMS_JS.read_text(encoding="utf-8"), re.S)
...
def write_dims(paths, changed=()):
    ...
    Sizes already in the file are reused: only images that are new, or whose
    previews were just re-encoded (`changed` — i.e. the source changed), are
    measured. Identifying all ~90 images on every save took several seconds and
    froze the editor. --force re-encodes, and so re-measures, everything.
    """
    old, changed = read_dims(), set(changed)
    dims = {}
    for rel in paths:
        src = BASE_DIR / rel
        if not src.exists():
            continue
        if rel in old and rel not in changed:
            dims[rel] = old[rel]
            continue
        try:
            r = subprocess.run(
                ["magick", "identify", "-ping", "-format", "%w %h", str(src) + "[0]"],
    ...
    text = ("// Auto-generated by make_previews.py — do not edit manually" + nl +
...
def main(argv=None) -> int:
    ...
    n_dims = write_dims(paths, changed)
    print(f"{DIMS_JS.name}: {n_dims} image dimension(s)")
